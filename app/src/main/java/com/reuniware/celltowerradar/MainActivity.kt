...
fun CellTowerRadarScreen(viewModel: MainViewModel = viewModel()) {
    ...
    // Simple refresh when component is shown
    LaunchedEffect(Unit) {
        val locationManager = context.getSystemService(Context.LOCATION_SERVICE) as android.location.LocationManager
        ...
        if (!isGpsEnabled) {
            Card(
                ...
                Row(modifier = Modifier.padding(12.dp), verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                    Text(
                        text = "GPS is DISABLED. Scanning will not work.",
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            ...
            if (isExpanded) {
                ...
                Text(text = "MCC: ${tower.mcc ?: "N/A"} | MNC: ${tower.mnc ?: "N/A"}")
                Text(text = "LAC/TAC: ${tower.lac ?: "N/A"} | CID/NCI: ${tower.cid ?: "N/A"}")
                ...
                Column(modifier = Modifier.padding(start = 8.dp)) {
                    ...
                    if (tower.type == "LTE") {
                        ...
                        tower.ta?.let { Text(text = "Timing Advance: $it", style = MaterialTheme.typography.bodySmall) }
                    }
